...
void token_add_single(char c, br_token_t token, br_token_list_t *list);
void token_add_atom(const char *buf, int start, int end, br_token_t token, br_token_list_t *list);
	...
	for (int i=0; i < bufsize; i++) {
		...
		switch (state) {
			case BR_STATE_INIT:
				...
				if (is_paren_open(c)) {
					token_add_single(c, BR_T_PAREN_OPEN, list);
					break;
				}
				if (is_paren_close(c)) {
					token_add_single(c, BR_T_PAREN_CLOSE, list);
					break;
				}
				if (is_atom(c)) {
					state = BR_STATE_ATOM;
			...
			case BR_STATE_NAME:
				if (is_char_ext(c)) {
					...
					token_add_atom(buf, start, i, BR_T_ID, list);
					state = BR_STATE_INIT;
					i--; // do not consume this char
				}
				break;
			case BR_STATE_NUM:
				if (is_digit(c)) {
					...
					token_add_atom(buf, start, i, BR_T_NUMBER, list);
					state = BR_STATE_INIT;
					i--; // do not consume this char
				}
				break;
			case BR_STATE_NUM_FLOAT:
				if (is_digit(c)) {
					...
					token_add_atom(buf, start, i, BR_T_FLOAT, list);
					state = BR_STATE_INIT;
					i--; // do not consume this char
				}
				break;
			case BR_STATE_STRING:
				if (is_string(c)) {
					token_add_atom(buf, start, i+1, BR_T_STRING, list);
					state = BR_STATE_INIT;
					break;
...
void token_add_single(char c, br_token_t token, br_token_list_t *list) {
	...
	token_list_insert(list, token, cc);
}
...
void token_add_atom(const char *buf, int start, int end, br_token_t token, br_token_list_t *list) {
	...
	t[len-1] = '\0';
	token_list_insert(list, token, t);
}
